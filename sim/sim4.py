...
from netsquid.examples.teleportation import EntanglingConnection
...
class PurifyExample(LocalProtocol):
    def __init__(self, node_a, node_b, num_runs, epsilon=0.9):
        ...
        self.subprotocols["distil_A"].start_expression = (
            self.subprotocols["distil_A"].await_signal(self.subprotocols["entangle_A"], Signals.SUCCESS))
        self.subprotocols["distil_B"].start_expression = (
            self.subprotocols["distil_B"].await_signal(self.subprotocols["entangle_B"], Signals.SUCCESS))
        self.subprotocols["filter_A"].start_expression = (
            self.subprotocols["filter_A"].await_signal(self.subprotocols["distil_A"], Signals.SUCCESS))
        self.subprotocols["filter_B"].start_expression = (
            self.subprotocols["filter_B"].await_signal(self.subprotocols["distil_B"], Signals.SUCCESS))
        self.subprotocols["teleport_A"].start_expression = (
            self.subprotocols["teleport_A"].await_signal(self.subprotocols["filter_A"], Signals.SUCCESS))
        self.subprotocols["teleport_B"].start_expression = (
            self.subprotocols["teleport_B"].await_signal(self.subprotocols["filter_B"], Signals.SUCCESS))
...
class EntanglingConnection(Connection):
    ...
    def __init__(self, length, source_delay, depolar_rate, name="EntanglingConnection"):
...
def example_network_setup(source_delay=1e5, source_fidelity_sq=1.0, depolar_rate=1000,
    ...
    qchannel =EntanglingConnection(length=node_distance, source_delay=source_delay, depolar_rate=depolar_rate)
...
def create_plot():
    ...
    fidelities.to_csv(f"{save_dir}/Distil&Filtering_Teleportation fidelity_{existing_files + 2}.csv")
