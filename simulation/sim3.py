...
def create_plot():
    ...
    save_dir = "./plots"
    existing_files = len([f for f in os.listdir(save_dir) if f.startswith("Teleportation fidelity with distil")])
    filename = f"{save_dir}/Teleportation fidelity with distil_{existing_files + 1}.png"
    data.plot(x='depolar_rate', y='fidelity', yerr='sem', **plot_style)
    plt.savefig(filename)
    print(f"Plot saved as {filename}")
    fidelities.to_csv(f"{save_dir}/Teleportation fidelity with distil_{existing_files + 1}.csv")
