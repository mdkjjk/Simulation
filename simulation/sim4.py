...
class PurifyExample(LocalProtocol):
    def __init__(self, node_a, node_b, num_runs, epsilon=0.9):
        ...
        self.subprotocols["teleport_A"].start_expression = self.subprotocols["teleport_A"].await_signal(self.subprotocols["filter_A"], Signals.SUCCESS)
        self.subprotocols["teleport_B"].start_expression = self.subprotocols["teleport_B"].await_signal(self.subprotocols["filter_B"], Signals.SUCCESS)
...
def example_network_setup(source_delay=1e5, source_fidelity_sq=1.0, depolar_rate=2500,
    ...
    cchannel = DirectConnection("CChannelConn_ter", ClassicalChannel("CChannel_dis_A->B", length=node_distance,
                                models={"delay_model": FibreDelayModel(c=200e3)}))
    network.add_connection(node_a, node_b, connection=cchannel, label="tereport", port_name_node1="cout_bob", port_name_node2="cin_alice")
...
def create_plot():
    ...
    save_dir = "./plots"
    existing_files = len([f for f in os.listdir(save_dir) if f.startswith("Teleportation fidelity with distil & filtering")])
    filename = f"{save_dir}/Teleportation fidelity with distil & filtering_{existing_files + 1}.png"
    data.plot(x='depolar_rate', y='fidelity', yerr='sem', **plot_style)
    plt.savefig(filename)
    print(f"Plot saved as {filename}")
    fidelities.to_csv(f"{save_dir}/Teleportation fidelity with distil & filtering_{existing_files + 2}.csv")
