...
class Example(LocalProtocol):
    def __init__(self, node_a, node_b, num_runs):
        ...
        self.add_subprotocol(BellMeasurement(node=node_a, port=node_a.ports["cout_bob"], qubit_protocol=InitState(node_a), name="teleport_A"))
...
class InitState(NodeProtocol):
    def __init__(self, node, name=None, start_expression=None):
...
class BellMeasurement(NodeProtocol):
    def __init__(self, node, port, qubit_protocol, start_expression=None, name=None):
        super().__init__(node, name)
        self.port = port
        self.qubit_protocol = qubit_protocol
        self.start_expression = start_expression
        self._qmem_pos0 = None
        self._qmem_pos1 = None
        self.add_subprotocol(self.qubit_protocol, "initstate")
        self.subprotocols["initstate"].start_expression = self.subprotocols["initstate"].await_signal(self, Signals.WAITING)
    ...
    def run(self):
        ...
        source_protocol = self.start_expression.atomic_source
        ready_signal = source_protocol.get_signal_by_event(event=self.start_expression.triggered_events[0], receiver=self)
        self._qmem_pos1 = ready_signal.result
        self.send_signal(Signals.WAITING)
        evexpr_signal = self.await_signal(self.subprotocols["initstate"], Signals.SUCCESS)
        yield evexpr_signal
        qubit_initialised = True
        self._qmem_pos0 = self.subprotocols["initstate"].get_signal_result(Signals.SUCCESS)
...
def example_sim_setup(node_a, node_b, num_runs):
    ...
    def record_run(evexpr):
        ...
        # Record fidelity
        q_A0, = node_a.qmemory.pop(positions=[result["pos_A0"]])
        q_A1, = node_a.qmemory.pop(positions=[result["pos_A1"]])
