...
class RecallFilter(Filter):
    ...
    def run(self):
        while self.attempts < self.iteration:
            print(f"{self.name}: Repeat start")
            self.attempts += 1
    ...
    def _check_fidelity(self):
        if self._qmem_pos is not None:
            q_state = self.node.qmemory.peek(positions=[self._qmem_pos])
            fidelity = qapi.fidelity(q_state, ks.b01, squared=True)
...
def create_plot():
    ...
    save_dir = "./plots"
    existing_files = len([f for f in os.listdir(save_dir) if f.startswith("Entanglement fidelity with filtering")])
    filename = f"{save_dir}/Entanglement fidelity with filtering_{existing_files + 1}.png"
