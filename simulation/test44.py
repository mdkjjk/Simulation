# シミュレーション：蒸留処理->フィルタリング処理を適用する場合のエンタングルメント忠実度の測定
...
class PurifyExample(LocalProtocol):
    def __init__(self, node_a, node_b, num_runs, epsilon=0.9):
        ...
        self.add_subprotocol(Filter(node_a, node_a.ports["cout_bob_fil"], epsilon=epsilon, name="filter_A1"))
        self.add_subprotocol(Filter(node_b, node_b.ports["cin_alice_fil"], epsilon=epsilon, name="filter_B1"))
        self.add_subprotocol(Filter(node_a, node_a.ports["cout_bob_fil"], epsilon=epsilon, name="filter_A2"))
        self.add_subprotocol(Filter(node_b, node_b.ports["cin_alice_fil"], epsilon=epsilon, name="filter_B2"))

        self.subprotocols["filter_A1"].start_expression = (
            self.subprotocols["filter_A1"].await_signal(self.subprotocols["entangle_A1"],
                                                       Signals.SUCCESS))
        self.subprotocols["filter_B1"].start_expression = (
            self.subprotocols["filter_B1"].await_signal(self.subprotocols["entangle_B1"],
                                                       Signals.SUCCESS))
        self.subprotocols["filter_A2"].start_expression = (
            self.subprotocols["filter_A2"].await_signal(self.subprotocols["entangle_A2"],
                                                       Signals.SUCCESS))
        self.subprotocols["filter_B2"].start_expression = (
            self.subprotocols["filter_B2"].await_signal(self.subprotocols["entangle_B2"],
                                                       Signals.SUCCESS))
        self.subprotocols["distil_A"].start_expression = (
            self.subprotocols["distil_A"].await_signal(self.subprotocols["filter_A1"], Signals.SUCCESS) |
            self.subprotocols["distil_A"].await_signal(self.subprotocols["filter_A2"], Signals.SUCCESS))
        self.subprotocols["distil_B"].start_expression = (
            self.subprotocols["distil_B"].await_signal(self.subprotocols["filter_B1"], Signals.SUCCESS) |
            self.subprotocols["distil_B"].await_signal(self.subprotocols["filter_B2"], Signals.SUCCESS))
        start_expr_ent_A1 = (self.subprotocols["entangle_A1"].await_signal(
                             self.subprotocols["distil_A"], Signals.FAIL) |
                             self.subprotocols["entangle_A1"].await_signal(
                             self.subprotocols["filter_A1"], Signals.FAIL) |
                             self.subprotocols["entangle_A1"].await_signal(
                                self, Signals.WAITING))
        self.subprotocols["entangle_A1"].start_expression = start_expr_ent_A1
        start_expr_ent_A2 = (self.subprotocols["entangle_A2"].await_signal(
                             self.subprotocols["distil_A"], Signals.FAIL) |
                             self.subprotocols["entangle_A2"].await_signal(
                             self.subprotocols["filter_A2"], Signals.FAIL) |
                             self.subprotocols["entangle_A2"].await_signal(
                                self, Signals.WAITING))
    ...
    def run(self):
        ...
        for i in range(self.num_runs):
            ...
            self.send_signal(Signals.WAITING)
            yield (self.await_signal(self.subprotocols["distil_A"], Signals.SUCCESS) &
                   self.await_signal(self.subprotocols["distil_B"], Signals.SUCCESS))
            signal_A = self.subprotocols["distil_A"].get_signal_result(Signals.SUCCESS,
                                                                       self)
            signal_B = self.subprotocols["distil_B"].get_signal_result(Signals.SUCCESS,
                                                                       self)
            result = {
                "pos_A": signal_A,
                "pos_B": signal_B,
                "time": sim_time() - start_time,
                "pairs": self.subprotocols["entangle_A1"].entangled_pairs,
            }
