...
class Filter(NodeProtocol):
    ...
    def _set_measurement_operators(self, epsilon): #測定演算子の設定
    ...
    def _handle_qubit_rx(self): #ノードが量子を受信すると実行
        # Handle incoming Qubit on this node.
        if self.node.qmemory.busy:
            yield self.await_program(self.node.qmemory)
        # Retrieve Qubit from input store
        output = self.node.qmemory.execute_instruction(INSTR_MEASURE, [self._qmem_pos], meas_operators=self.meas_ops)[0] #測定(0 or 1)
        if self.node.qmemory.busy:
            yield self.await_program(self.node.qmemory)
        m = output["instr"][0]
        # m = INSTR_MEASURE(self.node.qmemory, [self._qmem_pos], meas_operators=self.meas_ops)[0]
        self.local_qcount += 1
        self.local_meas_OK = (m == 0) #測定結果が成功か失敗かを示すフラグ
    ...
    def _check_success(self):
        ...
        if (self.local_qcount > 0 and self.local_qcount == self.remote_qcount and
                self.local_meas_OK and self.remote_meas_OK): #測定結果を比較
            # SUCCESS!
            self.send_signal(Signals.SUCCESS, self._qmem_pos) #一緒なら成功
            print(f"{self.name} protocol: {Signals.SUCCESS} sent with result: {self._qmem_pos} at time {sim_time()}")
            ...
            # FAILURE
            self._handle_fail()
            self.send_signal(Signals.FAIL, self.local_qcount) #違うなら失敗
            print(f"{self.name} protocol: {Signals.FAIL} sent with result: {self.local_qcount} at time {sim_time()}")

    def _handle_fail(self): #失敗した場合は、その量子状態を破棄
...
class InitStateProtocol(NodeProtocol):
    def run(self):
        ...
        print(f"{self.name}: Signal {Signals.SUCCESS} sent with result: {mem_pos} at time {sim_time()}")


class BellMeasurementProtocol(NodeProtocol):
    def __init__(self, node, qubit_protocol, name, start_expression=None):
        super().__init__(node, name)
        self.start_expression = start_expression
...
class FilteringExample(LocalProtocol):
    ...
    def __init__(self, node_a, node_b, num_runs, epsilon=0.3):
        ...
        self.add_subprotocol(BellMeasurementProtocol(node=node_a, qubit_protocol=InitStateProtocol(node_a), name="teleportation_A"))
        ...
        self.subprotocols["teleportation_A"].start_expression = (
            self.subprotocols["teleportation_A"].await_signal(self.subprotocols["purify_A"], Signals.SUCCESS))
        self.subprotocols["teleportation_B"].start_expression = (
            self.subprotocols["teleportation_B"].await_signal(self.subprotocols["purify_B"], Signals.SUCCESS))
